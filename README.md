...
Hiện repo đã cắt GA phiên bản 8.0.0 (v6-expansion — opt-in progressive deepening tier-2: `deepen` tier-2 tuỳ chọn `glossary`/`feature-spec`/`adr`/`test-strategy` dưới `docs/design/`, không đổi luồng tầng 1 mặc định) sau 7.0.0 (v1-fix-bugs — runtime integrity & newbie journey repair), và đang **RELEASE CANDIDATE** cho mốc 8.1.0 (Interactive Question Cards — thẻ chọn cho câu hỏi phỏng vấn, xem [InteractiveQuestionCardsPlan.md](Design/RoadMap/InteractiveQuestionCardsPlan.md) và [v8.1-release-note.md](Design/RoadMap/v8.1-release-note.md)), **chưa cắt GA**: 5/6 contract đã đóng có bằng chứng ([v7-expansion/README.md](Design/ContractForAI/Core/v7-expansion/README.md)), chỉ còn R-spike (`UserPromptSubmit` có bắn khi trả lời thẻ không) cần một phiên Claude Code thật — probe đã dựng sẵn, chờ chủ repo chạy. Số version có một nguồn sự thật là `package.json`, được `scripts/check-version-sync.mjs` kiểm mỗi lần `npm test`. Xem [v7-release-note.md](Design/RoadMap/v7-release-note.md) cho known limitations của 7.0.0 đã công bố.

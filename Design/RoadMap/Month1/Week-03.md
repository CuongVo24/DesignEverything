...
## Việc chi tiết
- [ ] `SessionStart`: khởi tạo `progress.json` ở S0 (gồm `answered_len_at_last_turn = 0`).
- [ ] `UserPromptSubmit`: **chỉ giới hạn nhịp + inject**, không advance — kiểm `answered` tăng ≤ 1/lượt người thật, cập nhật `answered_len_at_last_turn` (mô hình hai lớp, state-schema §3).
- [ ] Skill commit step ngữ nghĩa: append `answered`, set `branch` sau S6, tính bước kế (đây là lớp skill, không phải hook).
...
## Đầu vào / Phụ thuộc
Engine lõi từ tuần 2, đặc tả hook trong `claude-code.md`, `gate-policy`, fixture web và danh sách test case bắt buộc. Tuần này cũng phụ thuộc vào phạm vi MVP đã khóa ở tuần 1, nhất là quy ước "chỉ web" và "1 gate". Test hook bám tầng GATE ca biên trong [../../Conventions/TestStrategy.md](../../Conventions/TestStrategy.md) (chặn đúng khi thiếu doc, không chặn nhầm lượt hỏi tiếp).

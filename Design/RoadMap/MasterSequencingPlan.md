...
| **Gate A1** | Nâng 22 contract `v1-fix-bugs` PARTIAL → IMPLEMENTED (P2→P10 của `plan-v1-fix.md` §6) | ⏳ TODO | Cột Implementation `v1-fix-bugs/README.md` — hiện 13/24 (B1a, B1b, B1c, B1d, B2a, B2d, B2e, B3a, B3c, B3d, B4d, B4e, B4f) |
...
| ↳ A1-P7 | B3d/B3e — transactional tier-1/tier-2 activation | ✅ B3d Implementation đóng 2026-08-03 (đối chiếu code, không cần code mới) | B3d 2 mục còn lại đã hoá ra đã đúng từ trước (execution-state plan-validating, CLI output từ manifest) — có test xác nhận. B3e giữ nguyên PARTIAL — 3 mục còn lại (emitTier2 qua transaction kernel, commitDeepenAnswer full CAS, rerun-versioning) là viết lại kiến trúc deepen, hoãn theo quyết định chủ repo 2026-08-03 (rủi ro đụng test deepen có sẵn, giống vụ emit-gate ở A1-P6) |
| ↳ A1-P8 | B4a/B4b/B4c — hook/CLI production wiring | ⏳ Một phần đóng 2026-08-05 | X09 đóng trước đó. 2026-08-05: 2 mục cấu trúc CLOSED (tách `evaluatePreAction.ts` 771→orchestrator 115 dòng + `src/core/preAction/`; tách `cliOperations.ts` 1262→launcher 75 dòng + `src/adapters/shared/cliOps/`; cả hai behavior-preserving, 915/915 test xanh trước/sau). SessionStart manifest-order checklist đối chiếu code = CLOSED (không cần code mới). Sửa 1 ghi chú sai lệch ở B4b: "Core còn includes('cli.mjs')" (dòng 2026-07-30) thực ra đã sai từ 2026-07-26 (commit 561a543) — X03 đã FIXED đúng theo finding-coverage-matrix, chỉ ghi chú trong contract bị trôi. B4a/B4b/B4c vẫn PARTIAL — phần lớn checklist còn lại (UserPromptSubmit/PreToolUse mapping đầy đủ, install-manifest launcher path, envelope/exit-class coverage, matrix process proof) chưa đánh giá lại |

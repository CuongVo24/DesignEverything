...
| **Gate A1** | Nâng 22 contract `v1-fix-bugs` PARTIAL → IMPLEMENTED (P2→P10 của `plan-v1-fix.md` §6) | ⏳ TODO | Cột Implementation `v1-fix-bugs/README.md` — hiện 10/24 (B1a, B1b, B1c, B1d, B2a, B3c, B3d, B4d, B4e, B4f) |
...
| ↳ A1-P5 | B2d/B2e — gate snapshot + runtime health | ⏳ Một phần đóng 2026-08-03 | R03 FIXED, X15 FIXED (phần B2e/profile). Còn mở: `execution-plan.json`/`gate-policy.yaml`/catalog chưa vào `inspectRuntimeHealth`; symlink escape qua `gateSnapshot.ts` chưa rà; B2d/B2e Implementation vẫn PARTIAL |

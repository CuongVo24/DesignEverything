...
| D-platform | Nền tảng và chiến lược client | React Native native CLI (không dùng Expo) để dễ dàng tích hợp các thư viện native chuyên sâu cho bảo mật. | M1 | `05-architecture.md` |
| D-overview | Hướng triển khai tổng quan (offline/sync) | Offline-first. Sử dụng WatermelonDB để đồng bộ hóa hiệu năng cao với PostgreSQL database qua REST API. | M2 | `05-architecture.md` |
| D-distribution | Phân phối và phát hành store | Quyền truy cập Photo Library và Files để tải file tài liệu đính kèm công việc. | M3 | `07-release.md` |
| D-auth | Xác thực và phân quyền | Cần FCM push notifications gửi ngay lập tức khi có task được giao hoặc có thay đổi khẩn cấp. | M4 | `05-architecture.md` |
| D-realtime | Realtime, push hoặc đồng bộ | Phát hành qua Apple Enterprise Program và Google Play Console Beta Testing để phục vụ nội bộ doanh nghiệp trước khi phát hành rộng rãi (mô hình thu phí B2B s… | M5 | `05-architecture.md` |

...
|---|---|---|---|---|
| D-shape | Hình-hài dự án | web | S7 | `06-constraints.md` |
...
| D-rendering | Giao diện và chiến lược rendering | React + Vite (SPA) kết hợp Supabase client-side API để triển khai nhanh gọn. | W1/W2 | `05-architecture.md` |
| D-overview | Hướng triển khai tổng quan | Responsive layout sử dụng CSS Grid để co giãn trên mobile. | W2 | `05-architecture.md` |
| D-hosting | Hosting và đường phát hành | Triển khai frontend lên Vercel free-tier, database trên Supabase free-tier. | W3 | `07-deployment.md` |
| D-auth | Xác thực và phân quyền | Supabase Auth (Email/Password và Google OAuth). | W4 | `05-architecture.md` |
| D-realtime | Realtime, push hoặc đồng bộ | Reload trang để cập nhật trạng thái; không cần WebSocket/Realtime ở MVP. | W5 | `05-architecture.md` |
<!-- anchor: id=decisions/table  src=src/features/decisions/decisions.ts::decisionTable  rev=  status=planned -->
...
<!-- anchor: id=decisions/rationale  src=src/features/architecture/architecture.ts::architectureDecisionRationale  rev=  status=planned -->
...
<!-- anchor: id=decisions/alternatives  src=src/features/architecture/architecture.ts::architectureAlternativesConsidered  rev=  status=planned -->
...
<!-- anchor: id=decisions/change-policy  src=src/features/decisions/decisions.ts::decisionChangePolicy  rev=  status=planned -->

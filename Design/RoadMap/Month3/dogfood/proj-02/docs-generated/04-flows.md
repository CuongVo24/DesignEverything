## Tại sao cần file này
File này biến danh sách tính năng thành một lần dùng thật từ đầu tới cuối. Nếu flow không trơn, rất có thể scope hoặc data model vẫn chưa chốt đúng.

## Luồng Điển Hình
Đăng nhập -> Thêm sách (tên, tác giả, số trang) -> Cập nhật trạng thái (chưa đọc, đang đọc, đã đọc) -> Ghi nhận trang sách hiện tại.
<!-- anchor: id=04-flows/main-flow-summary  src=src/features/flows/flows.ts::mainFlowSummary  rev=  status=planned -->

## Các Bước Chính
Đăng nhập -> Thêm sách (tên, tác giả, số trang) -> Cập nhật trạng thái (chưa đọc, đang đọc, đã đọc) -> Ghi nhận trang sách hiện tại.
<!-- anchor: id=04-flows/main-flow-steps  src=src/features/flows/flows.ts::mainFlowSteps  rev=  status=planned -->

## Sơ Đồ Luồng Chính
```mermaid
graph LR
  S0["Đăng nhập"]
  S1["Thêm sách (tên, tác giả, số trang)"]
  S2["Cập nhật trạng thái (chưa đọc, đang đọc, đã đọc)"]
  S3["Ghi nhận trang sách hiện tại."]
  S0 --> S1
  S1 --> S2
  S2 --> S3
```

Đây là luồng chính — cũng chính là thứ phải chạy được end-to-end ở milestone M0 (xem `08-build-plan.md`). Mỗi lần xong một milestone, chạy lại đúng luồng này như một người dùng thật.
<!-- anchor: id=04-flows/diagram  src=src/features/flows/flows.ts::flowDiagram  rev=  status=planned -->

## Điểm Dễ Vỡ Hoặc Cần Làm Rõ
Đăng nhập -> Thêm sách (tên, tác giả, số trang) -> Cập nhật trạng thái (chưa đọc, đang đọc, đã đọc) -> Ghi nhận trang sách hiện tại.
<!-- anchor: id=04-flows/flow-risks  src=src/features/flows/flows.ts::flowRisks  rev=  status=planned -->

...
## 3. Checklist
- [ ] Mỗi file emit khớp template tương ứng; placeholder thay hết.
- [ ] Mỗi mục có anchor đúng format (`status=planned`, rev trống).
- [ ] Nhánh web emit `07-deployment.md`, KHÔNG có `07-release.md` (và ngược lại).
- [ ] Tên file ∈ taxonomy; mỗi file có "## Tại sao cần file này".
- [ ] Test EMIT trên dữ liệu golden.

## 4. Interfaces / Files expected to change
```ts
export function emitDoc(targetDoc: string, filledSlots: Record<string,string>): string;
export function emitTree(answers: InterviewAnswers, branch: 'web'|'mobile'): { file: string; content: string }[];
```

...
## 3. Checklist
- [ ] 3 file schema + `index.ts` export type qua `z.infer`.
- [ ] `answered_len_at_last_turn` có trong `progressSchema`.
- [ ] `branch` enum + nullable đúng cả ở question (core/web/mobile) và state (web/mobile/null).
- [ ] Không khai báo `interface`/`type` trùng shape ở nơi khác — chỉ suy từ zod.

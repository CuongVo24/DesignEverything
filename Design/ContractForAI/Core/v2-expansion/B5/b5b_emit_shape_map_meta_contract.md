...
## 3. Checklist
- [ ] `emitTree(branch: string)` validate ∈ registry; branch lạ → lỗi rõ.
- [ ] File-map đọc từ `shapes.yaml` (không hardcode if/else 07-*).
- [ ] `cli` emit đúng cây: 00–06 + `07-distribution.md` + README, slot C1–C5 có nội dung.
- [ ] Câu meta không sinh doc/anchor.
- [ ] web/mobile/hybrid output **byte-khớp** hành vi cũ (golden xanh).

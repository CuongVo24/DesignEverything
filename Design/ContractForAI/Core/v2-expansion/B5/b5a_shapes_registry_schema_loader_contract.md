...
## 3. Checklist
- [x] `shapes.yaml` tồn tại, parse+validate, khớp 4 shape trong taxonomy registry.
- [x] zod `interviewScript`: `branch` string theo registry; `kind` default `anchored`; `target_doc` null-iff-meta (refine).
- [x] zod `state`: `branch` string|null theo registry.
- [x] `loadShapes` + validator hoạt động; script.yaml **hiện tại** (chưa có kind/meta) vẫn validate (tương thích ngược).
- [x] `state`: `calibrate_mode` ∈ {deep,fast,null}. `critics` (nếu có) key ∈ question ids, entry `{challenge, ack_prompt}` không rỗng.
- [x] `contentIntegrity` kiểm shapes.yaml ↔ taxonomy + branch hợp lệ.

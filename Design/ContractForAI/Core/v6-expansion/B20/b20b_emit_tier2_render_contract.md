...
- [NEW] `src/core/schemas/tier2Render.ts` (types), `src/core/tier2RenderHelpers.ts` (dựng khối + anchor + SourceRef + đếm unknown), 4 renderer thuần `renderGlossary/renderFeatureSpec/renderAdr/renderTestStrategy.ts`, `src/core/emitTier2.ts` (transaction) + 2 test (`renderTier2`, `emitTier2`).
- [MODIFY] `checkDocsConsistency.ts` thêm `checkTier2Consistency` (feature ∉ Must → `error`, entity lạ → `warning`), export ở `index.ts`.
- **Scheme SourceRef renderer khoá**: cite đúng key kho answers — `answers:DS1a`/`DS1b`, `answers:DS2a@<slug>` (theo instance từng Must), `answers:DS3a@adr-NNN` (subject positional), `answers:DS4a`/`DS4b`; dữ kiện tầng 1 → `doc:docs/..#<id-có-tiền-tố-số>`; thiếu nguồn → `⚠ unknown` (không bịa).
- **Transaction** đúng thứ tự: opt-in/đủ-câu → render thuần (in-memory) → consistency → chỉ khi sạch `error` mới ghi file (atomic tmp+rename) + dọn file mồ côi theo manifest + cập nhật `emitted_at`/`source_digest`/`artifacts` và `saveDeepenState` một lần. `consistency-error`/`missing-answers`/`not-opted-in` → skipped, không ghi gì.
- Verify: `npx vitest run renderTier2 emitTier2` → 13 pass (đủ 1-file-mỗi-Must + slug khớp slugify, DS2 A không rò sang B, ADR numbering positional ổn định + dịch số khi bỏ quyết định, cờ unknown khi thiếu DS, re-emit dọn file mồ côi, consistency error/warning). `npm run lint`/`build` sạch, `npm test` → **323 pass / 61 file**, golden/dogfood tầng 1 không đổi.

...
| P6 — answer/provenance/catalog | Partial (2026-07-26) | `commitInterviewAnswer` giờ load `answer_contract` thật của câu hỏi hiện tại (trước đây luôn `null`) và enforce `pattern`/`enum_values`/`required`/`warning_rules`→`needs_user_ack` qua `--ack-warnings`; `min_items`/`required_fields` (structured slots), derived provenance (10.2) và catalog consumers (10.3) vẫn chưa |
| P7 — transactional emit | Partial (2026-07-26) | Production `emit` giờ gọi `activateTier1Emit` (render→stage→validate→activate qua kernel có sẵn), thay direct-write loop và catch-branch biến lỗi thành success đã xóa hẳn. `activateTier1Emit` và `completeTier1Activation` vẫn là hai lời gọi riêng (không cùng một transaction/journal); tier-2 isolation (P7.2) và fault-injection matrix (11.3) vẫn chưa |

...
- [x] Zod schema reject task/evidence không hợp lệ, duplicate evidence và transition không đúng.
- [x] Evidence append-only; repair gắn với task hiện tại.
- [x] Gate không mở nếu chỉ có docs đúng tên mà không có validation pass.
- [x] Task khác active_task hoặc path ngoài allows_paths bị deny ở Bậc A.
- [x] State hỗ trợ resume sau phiên mới bằng evidence đã lưu.

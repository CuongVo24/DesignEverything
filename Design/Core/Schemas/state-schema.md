...
> Vì sao chia vậy: hook **không thể** phân loại web/mobile hay chấm "câu trả lời hay/dở" từ text thô (xem [gate-policy.md](gate-policy.md) §1). Bắt nó làm thế là thiết kế sai. Đảm bảo cứng mà sản phẩm hứa = (1) gate dựa artifact + (2) một-bước-mỗi-lượt-người-thật — KHÔNG phải "hook validate từng câu".

## 4. Quy tắc chuyển bước (ai làm gì)

1. **`SessionStart` (hook)** khởi tạo state với:
   - `phase = "interview"`, `branch = null`, `current_step = "S0"`
   - các mảng rỗng
...
4. **Rẽ nhánh sau `S6` — skill set, hook validate.** Khi commit `S6`, skill gán `branch = "web"` hoặc `"mobile"` từ câu trả lời đã chốt, rồi chọn `current_step` đầu nhánh (`W1` hoặc `M1`). Hook chỉ kiểm `branch ∈ {web, mobile}` và chỉ được set đúng một lần (xem §5).
...
- `branch` chỉ nhận `web`, `mobile`, hoặc `null`; một khi đã khác `null` thì **không được đổi** trong cùng phiên — rẽ nhánh là quyết định một chiều, muốn đổi nhánh phải sửa doc có chủ đích rồi chỉnh state tường minh.

...
→ INJECT/EMIT gần như giống nhau khắp nơi. **Chỉ GATE khác** và xuống bậc theo năng lực harness. Target 4.0.0 thêm ORCHESTRATE: adapter chỉ nối host với core state/CLI để đọc task active, validate/start/record/repair; không tự chế task, business logic hay agent mới.
...
### Plan validation & execution evidence (target 4.0.0)

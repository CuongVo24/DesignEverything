...
| 8.1.1 | **chưa cắt** | v8-hotfix — H1–H6: đóng bế tắc bootstrap, `--slots-file`, gate PowerShell, `status` question card, `gates_passed`/`ready-for-validation`. 6/6 contract DONE 2026-08-16. **Version chưa bump** — `package.json` vẫn 8.1.0, chưa có tag. Xem [v8-hotfix/](../ContractForAI/Core/v8-hotfix/) và Gate D1 ở [MasterSequencingPlan.md](../RoadMap/MasterSequencingPlan.md). | Không có |
| 8.2.0 | **chưa cắt** | v8-expansion — Interview Cadence: `undo` một bước, batch nhiều câu một lượt do Core quyết, `multi_select` (D59–D61). 8/8 contract DONE 2026-08-16. **MINOR.** **Version chưa bump** — `package.json` vẫn 8.1.0, chưa có tag. Xem [InterviewCadencePlan.md](../RoadMap/InterviewCadencePlan.md). | Claude Code + Codex (đồng bộ văn xuôi, khác mức enforcement) |

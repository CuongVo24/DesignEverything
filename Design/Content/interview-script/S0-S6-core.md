...
## R1 — Khảo sát bất định và rủi ro

**ask**  
"Dự án của bạn có rủi ro hay bất định nào sau đây không: phụ thuộc thư viện/API bên ngoài, chi phí vận hành/tài khoản Store/Cloud, dữ liệu nhạy cảm cần bảo mật, quyền thiết bị phức tạp, hay có phần nào bạn chưa biết cách làm không? (Bạn có thể trả lời 'không biết' hoặc bỏ qua)."

**default**  
`Không biết/Cần khảo sát thêm.`

**translate_back**  
"Mình ghi nhận các điểm bất định/rủi ro kỹ thuật cần khảo sát: `<rủi ro của bạn>`. Mình sẽ tự động phân loại chúng thành confirmed (đã xác thực), assumption (giả định), hoặc spike-required (cần nghiên cứu trước)."
...
**Ví dụ trả lời đời thường**  
"Mình cần dùng API Google Maps nhưng chưa rõ chi phí free tier thế nào, và cũng chưa biết cách nhúng bản đồ vào React."

**Ví dụ dịch ngược**  
"Phát hiện rủi ro: Google Maps API (Phụ thuộc bên ngoài & Chi phí vận hành, status: assumption); Nhúng bản đồ vào React (Điều chưa biết, status: spike-required)."

**Bẫy thường gặp + cách gỡ**  
Người mới thường bỏ qua rủi ro hoặc xem giả định kỹ thuật là sự thật hiển nhiên. Agent phải phân loại rõ ràng và đề xuất "Feasibility Spike" thay vì tự động điền câu trả lời hoặc giả định dự án tự chạy được.

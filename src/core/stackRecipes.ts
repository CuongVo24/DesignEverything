    ...
    return {
      ...
      tasks: {
        ...
        T1: {
          ...
          type: 'scaffold',
          milestone: 'M1-scaffold',
          intent: 'Initialize the Vite manifest and install the Vite build dependency.',
          depends_on: ['T0-discovery'],
          allowed_paths: manifests,
          preconditions: ['T0-discovery verified'],
          commands: [
            ...
            {
              id: 'install-vite',
              argv: [pm, 'install', '--save-dev', 'vite'],
              expected: { kind: 'file-exists', value: 'node_modules/vite/package.json' },
              requires_user_confirmation: true,
            },
          ],
          expected_result: 'package.json exists, vite is in devDependencies, and the agent adds a build script plus index.html and vite.config.* within the allowed manifests.',
          evidence_required: ['init-project', 'install-vite'],
          failure_policy: 'abort',
          requires_capability: 'node-npm-project',
        },
        T2: {
          ...
          preconditions: ['T1-scaffold completed'],
          commands: [
            { id: 'check-entry', argv: ['node', '-e', "process.exit(require('fs').existsSync('index.html') ? 0 : 1)"], expected: { kind: 'exit-code-zero' } },
          ],

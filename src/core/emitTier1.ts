...
export function activateTier1Emit(
  ...
  // A1-03 (Wave A1) — from here on a staging directory exists on disk
  // (prepareEmit above). Every exit path, success or reject, must clean it
  // up: activateEmit COPIES staged files into the live tree rather than
  // moving/renaming the staging dir (emitTransactionActivate.ts), so
  // nothing else ever deletes it — recoverEmit's crash recovery reads only
  // the journal/backups, never the staging dir, so removing it here can't
  // break recovery. Best-effort: a cleanup failure must never mask the
  // real result below.
  try {
    return activateStagedTier1Generation(workspaceRoot, generation, catalog, derivedRecipes, handoffInput);
  } finally {
    try {
      rmSync(generation.stagingDir, { recursive: true, force: true });
    } catch {
      // Best-effort — an orphaned staging dir is a cleanup nicety, not a
      // correctness issue; it must never be reported as an emit failure.
    }
  }
}

function activateStagedTier1Generation(
  workspaceRoot: string,
  generation: StagedGeneration,
  catalog: RuntimeCatalog,
  derivedRecipes: ReturnType<typeof loadDerivedRecipes>['recipes'],
  handoffInput: Tier1EmitHandoffInput
): ActivateTier1EmitResult {

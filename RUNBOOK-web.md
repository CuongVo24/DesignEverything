# Runbook — Hướng Dẫn Vận Hành Nhánh Web

Runbook này cung cấp đầy đủ các chỉ dẫn chuẩn bị, các câu lệnh vận hành hệ thống phỏng vấn và kiểm chứng cổng chặn (gating) của nhánh **Web** trong khuôn khổ Month 1.
...
## 2. Quy Trình Vận Hành Demo Phỏng Vấn (Tái lập E2E)
...
Lần lượt trả lời các câu hỏi từ S0 đến S6. Tại bước S6, người dùng chọn nhánh `'web'`.
...
*   **Đổi nhánh**: Nhánh đã chọn tại S6 là `'web'` sẽ được khóa chặt. Bất kỳ nỗ lực đổi sang nhánh `'mobile'` nào sau đó đều bị báo lỗi và từ chối.

### Mốc 3: Chặn viết code khi thiếu tài liệu (Gate Blocked)
Tại bước phỏng vấn **W1** (khi phỏng vấn chưa kết thúc và chưa sinh docs):
*   **Thử viết code**: Ghi file `src/index.ts` hoặc cài package `npm install`.
...
Tiếp tục phỏng vấn đến **W5**. Sau khi commit W5, trạng thái cập nhật `current_step = null`, `phase = "docs-emitted"`.
*   **Sinh tài liệu**: AI gọi động cơ phát hành tài liệu `emitTree` trên tập câu trả lời đã thu thập.
*   **Kết quả**: Sinh chính xác 9 file tài liệu đặt vào thư mục `docs/`.

### Mốc 5: Emit xong — chưa mở cổng (Handoff Truth)
*   **Hành động**: `emit` sinh xong `docs/`; AI gọi lại lệnh viết code `Write src/index.ts` ngay.
...
## 3. Bảng Điểm Đánh Giá Chất Lượng (Quality Rubric Scorecard)
...
| **Quyết định kiến trúc khớp nhánh** | `05-architecture.md` | Các quyết định Next.js SSR/CSR, Responsive Web, Auth đều lấy từ câu trả lời nhánh Web. | ĐẠT | 10/10 |
...
| **Đường phát hành Web** | `07-deployment.md` | Chi tiết quy trình deploy lên hosting Vercel free-tier (không bị nhầm lẫn sang mobile release). | ĐẠT | 10/10 |
...
```bash
# Chạy toàn bộ các ca unit test và E2E
npm test
